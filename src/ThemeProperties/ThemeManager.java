...
import JWindowPlus.JWindowPlus;
...
public class ThemeManager {
    ...
    public static Properties getDarkThemeDefault() {
        return PropertyUtils.loadPropertiesFromJar("resources/DarkTheme.properties");
    }

    public static Properties getLightThemeDefault() {
        return PropertyUtils.loadPropertiesFromJar("resources/LightTheme.properties");
    }
    ...
    public void setTheme(Container content) {
        ...
        if (content instanceof JWindowPlus windowPlus) {
    ...
    private void applyWindowTheme(JWindowPlus window) {
        boolean isDarkTheme = ThemeUtils.isColorDark(theme.getColor(theme.PRIMARY));
        window.setBackgroundPlus(
                ThemeUtils.adjustColor(theme.getColor(theme.PRIMARY), 0, isDarkTheme)
        );
    ...
    private void applyWindowThemeByTone(JWindowPlus window) {
        boolean isDarkTheme = ThemeUtils.isColorDark(theme.getColor(theme.PRIMARY));
        window.setBackgroundPlus(
    ...
    public boolean loadThemeFromCache(String key) {
    Theme cachedTheme = cache.get(key);
    if (cachedTheme != null) {
        theme = cachedTheme;
        themeFilePath = key;
        return true;
